...
def transform_resale_flats(filename, df_districts):
    ...
    def get_info_from_street_name(address):
        ...
        # else call the api and add to dict1
            ...
            url = "https://developers.onemap.sg/commonapi/search?searchVal={}&returnGeom=Y&getAddrDetails=Y".format(
                address
            )
    ...
    resale_flats["street_name_with_block"] = (
        resale_flats["block"] + " " + resale_flats["street_name"]
    )
    (
        resale_flats["postal"],
        resale_flats["x"],
        ...
        resale_flats["lat"],
        resale_flats["lon"],
    ) = zip(*resale_flats["street_name_with_block"].apply(get_info_from_street_name))
    ...
    def get_district_from_postal(postal):
        ...
        value = districts[districts["Postal Sector"] == postal_sector][
            "Postal District"
        ].values
        if not len(value):
            print(postal)
    ...
    df = pd.DataFrame.from_dict(
        dict1, orient="index", columns=["postal", "x", "y", "lat", "lon"]
    )
...
def transform_private_transactions_and_rental(
    filename_private_transactions, filename_private_rental
):
...
def transform_salesperson_transactions(filename, df_salesperson_info_filename):
    ...
    salesperson_transactions["district"] = salesperson_transactions["district"].replace(
        "-", "NULL"
    )

    # for transaction_date column
    salesperson_transactions["transaction_date"] = salesperson_transactions[
        "transaction_date"
    ].str.split("-")
    salesperson_transactions["month"] = salesperson_transactions[
        "transaction_date"
    ].apply(lambda x: x[0])
    # for the year, add prefix 20
    salesperson_transactions["year"] = salesperson_transactions[
        "transaction_date"
    ].apply(lambda x: x[1])

    # drop transaction_date column
    salesperson_transactions = salesperson_transactions.drop(
    ...
    salesperson_transactions = salesperson_transactions[
        salesperson_transactions["salesperson_reg_num"].isin(
            df_salesperson_info["registration_no"]
        )
    ]
    # remove where district is not in the range 1-28
    salesperson_transactions = salesperson_transactions[
        salesperson_transactions["district"].isin(range(1, 29))
    ]
...
def transform_rental_flats(filename, districts):
    ...
    # Apply get_info_from_street_name function to each row using a new column created by concat block and street name
    rental_flats["street_name_with_block"] = (
        rental_flats["block"] + " " + rental_flats["street_name"]
    )
    (
        rental_flats["postal"],
        rental_flats["x"],
        ...
        rental_flats["lat"],
        rental_flats["lon"],
    ) = zip(*rental_flats["street_name_with_block"].apply(get_info_from_street_name))
